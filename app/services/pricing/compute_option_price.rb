module Pricing
  class ComputeOptionPrice
    ...
    def calc_one_option_price_per_sq_ft(openings, option, shape)
      area = 0
      if option.minimum_quantity.zero?
        ...
        when Option::PRICED_PER_SECTION # per section
          1.upto(shape.sections_height) do |r|
            1.upto(shape.sections_width) do |c|
              ...
              area += @quotation_line.compute_minimum_section_area(
                @line_info.real_height[r] * @line_info.real_width[c] / 144.0, option, opening
              )
            end
          end
          if shape.has_upper_transom?
            ...
            area += @quotation_line.compute_minimum_section_area(
              @line_info.section_height[index].to_f * @line_info.total_width / 144.0, option, opening
            )
          end
          ...
          if shape.has_left_sidelight?
            ...
            opening = Opening.find(openings[index].to_i)
            area += @quotation_line.compute_minimum_section_area(
              @line_info.section_height[index].to_f * @line_info.section_width[index].to_f / 144.0, option, opening
            )
          end
      ...
      if option.minimum_quantity.zero?
        ...
        when Option::PRICED_PER_GLASS # per glass
          ...
          1.upto(shape.sections_height) do |r|
            1.upto(shape.sections_width) do |c|
              opening = Opening.find(openings[(((r - 1) * shape.sections_width) + c).to_s].to_i)
              glasses_quantity = (opening.glasses_quantity.zero? ? 1 : opening.glasses_quantity)
    ...
    def compute_area_for_openings(shape, openings, openable)
      ...
      if shape.has_upper_transom?
        area += compute_section_area(@line_info.upper_transom_index, openable, openings,
                                     @line_info.total_width)
      end
      if shape.has_lower_transom?
        area += compute_section_area(@line_info.lower_transom_index, openable, openings,
                                     @line_info.total_width)
      end
