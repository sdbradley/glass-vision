module QuotationLineHelper
  def select_opening(section, selected)
    ...
    output << select_tag("openings_combo_#{section}", options_for_select([['----', 0]] + @quotation_line.serie.openings.map do |o|
                                                                                         ...
                                                                                         end, selected), onchange: "$('#openings_#{section}').val($(this).value); $('#opening_pic_#{section}').attr('src', pics_#{section}['img_' + $(this).val()]);")
    output << '<![endif]-->'
    output << '</div>'
    output.html_safe
  end
  ...
  def size_tag_helper(width_or_height, idx)
    if @quotation_line.serie.is_standard_product?
      select_tag("section_#{width_or_height}[#{idx}]",
                 options_from_collection_for_select(@serie.width_or_height, 'value', 'value'))
    else
