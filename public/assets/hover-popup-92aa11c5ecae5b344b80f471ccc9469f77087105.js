(() => {
  ...
  (function($) {
    $.fn.thumbPopup = function(options) {
      ...
      $(this).hover(setPopup).mousemove(updatePopupPosition).mouseout(hidePopup);
      function setPopup(event) {
        var fullImgURL = $(this).attr("src").replace(settings.imgSmallFlag, settings.imgLargeFlag);
        $(this).data("hovered", true);
        $("<img />").bind("load", { thumbImage: this }, function(event2) {
          if ($(event2.data.thumbImage).data("hovered") == true) {
            $(popup).empty().append(this);
            updatePopupPosition(event2);
            $(popup).show();
          }
          $(event2.data.thumbImage).data("cached", true);
        }).attr("src", fullImgURL);
